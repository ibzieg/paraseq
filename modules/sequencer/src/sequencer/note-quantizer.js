...
class NoteQuantizer {
  ...
  static get activeScale() {
    ...
    let rootNoteIndex =
      NoteQuantizer.getNoteIndexFromName(scene.options.root) % 11;
    let modeIndex = NoteQuantizer.getModeIndexFromName(scene.options.mode) % 6;
    return NoteQuantizer.generateScaleFromIndex(
      rootNoteIndex,
      modeIndex,
      false
    );
  ...
  static isTriadMemberOfScale(triad) {
    if (
      NoteQuantizer.activeScale.indexOf(triad[0]) >= 0 &&
  ...
  static getHarmonizedChord(noteIndex) {
    ...
    let majorTriad = [majorScale[0], majorScale[2], majorScale[4]];
    ...
    const minorTriad = [minorScale[0], minorScale[2], minorScale[4]];
  ...
  static makeChord(value) {
    ...
    let chord = NoteQuantizer.getHarmonizedChord(
      note,
      NoteQuantizer.activeScale
    );
    ...
    if (chord) {
      ...
      Log.debug(
        `makeChord: no chord exists for note=${note} name=${
          NoteQuantizer.activeScale[note]
        } scale=${NoteQuantizer.activeScale}`
      );
      return [];
    }
  }
